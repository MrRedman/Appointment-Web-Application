...
namespace Appointment_System.Models
{
    public class DiaryEvent
    {
        ...
        public static List<DiaryEvent> LoadAppointmentSummaryInDateRange(double start, double end)
        {
            ...
            using (DiaryContainer ent = new DiaryContainer())
            {
                var rslt = ent.AppointmentDiaries.Where(s => s.DateTimeSchedule >= fromDate && System.Data.Objects.EntityFunctions.AddMinutes(s.DateTimeSchedule, s.AppointmentLength) <= toDate)
                                                        .GroupBy(s => System.Data.Objects.EntityFunctions.TruncateTime(s.DateTimeSchedule))
        ...
        public static void UpdateDiaryEvent(int id, string NewEventStart, string NewEventEnd) 
        {
            ...
            using (DiaryContainer ent = new DiaryContainer()) {
                ...
                if (rec != null)
                {
                    ...
                    rec.DateTimeSchedule = DateTimeStart;
        ...
        public static bool CreateNewEvent(string Title, string NewEventDate, string NewEventTime, string NewEventDuration)
        {
            try
            {
                ...
                rec.Title = Title;
                rec.DateTimeSchedule = DateTime.ParseExact(NewEventDate + " " + NewEventTime, "dd/MM/yyyy HH:mm", CultureInfo.InvariantCulture);
